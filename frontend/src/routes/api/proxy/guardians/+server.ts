...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		try {
			...
			const httpRequest = [
				`GET /api/settings/guardians HTTP/1.1`,
				`Host: ${host}`,
...
export const POST: RequestHandler = async ({ url, request }) => {
	...
	try {
		...
		try {
			...
			const httpRequest = [
				`POST /api/settings/guardians HTTP/1.1`,
				`Host: ${host}`,
				`Authorization: Bearer ${token}`,
...
export const PATCH: RequestHandler = async ({ url, request }) => {
	...
	try {
		...
		try {
			...
			const httpRequest = [
				`PATCH /api/settings/guardians HTTP/1.1`,
				`Host: ${host}`,
				`Authorization: Bearer ${token}`,
				`Content-Type: application/json`,
				`Content-Length: ${payloadStr.length}`,
...
export const DELETE: RequestHandler = async ({ url }) => {
	...
	try {
		...
		try {
			...
			const httpRequest = [
				`DELETE /api/settings/guardians?id=${id} HTTP/1.1`,
				`Host: ${host}`,
				`Authorization: Bearer ${token}`,
				`Connection: close`,
				``,
