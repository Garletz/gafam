...
export const GET: RequestHandler = async ({ url }) => {
	...
	const paths: Record<string, string> = {
		status: `/api/web/suparna/status?token=${encodeURIComponent(token)}`,
		'sms-codes': `/api/web/suparna/sms-codes?token=${encodeURIComponent(token)}`
	};
	const path = paths[action];
	if (!path) return json({ error: 'Unknown action' }, { status: 400 });
...
export const POST: RequestHandler = async ({ url }) => {
	const vpcUrl = url.searchParams.get('vpcUrl');
	const token = url.searchParams.get('token');
	const action = url.searchParams.get('action') || 'read-logs';
	const day = url.searchParams.get('day');
	const refresh = url.searchParams.get('refresh') || '0';
	const llm = url.searchParams.get('llm') || '0';
	...
	if (action === 'read-logs') {
		...
		path = `/api/web/logs/suparna?token=${encodeURIComponent(token)}&day=${encodeURIComponent(day)}&refresh=${refresh}&llm=${llm}`;
	} else if (action === 'download-model') {
		path = `/api/web/suparna/model?token=${encodeURIComponent(token)}`;
	} else {
