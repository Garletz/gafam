...
export const POST: RequestHandler = async ({ url, request }) => {
	...
	try {
		...
		try {
			...
			if (statusCode !== 200) return json({ error: `VPC returned HTTP ${statusCode}` }, { status: statusCode });

			return json(JSON.parse(body));

		} catch (socketError: any) {
			try {
				const response = await fetch(`${vpcUrl}/api/web/sms/outbox?token=${encodeURIComponent(token)}`, {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: payload
				});

				if (!response.ok) return json({ error: 'VPC fetch fallback error' }, { status: response.status });
