...
func buildPlannerPrompt(instruction string, maxQuests int) (system, user string) {
	...
	b.WriteString("🌐 MCP browser (browser.mcp_*) — the SHARED Chrome session, SAME logins as the human:\n")
	b.WriteString("   browser.mcp_snapshot → accessibility tree of the page (SEE it, no vision needed)\n")
	b.WriteString("   browser.mcp_navigate / browser.mcp_click / browser.mcp_type / browser.mcp_fill_form\n")
	b.WriteString("   browser.mcp_wait_for (wait for text/element — e.g. an email OTP) / browser.mcp_tabs\n")
	b.WriteString("   browser.mcp_take_screenshot / browser.mcp_network_requests\n")
	b.WriteString("   → PREFER these for LOGGED-IN tasks (Gmail OTP, booking, accounts): the agent\n")
	b.WriteString("     works in the same persistent profile the human sees on the dashboard.\n")
	b.WriteString("     Work in a DEDICATED tab (browser.mcp_tabs new) so the human's tabs stay theirs.\n")
	...
	b.WriteString("   contacts.auto_analyze() → enrich contacts with new SMS (profession/skills)\n")
