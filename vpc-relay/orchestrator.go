...
func buildPlannerPrompt(instruction string, maxQuests int) (system, user string) {
	...
	b.WriteString("   browser.fetch  → GET URL, returns page text\n")
	b.WriteString("   browser.sense  → fetch + LLM extraction in one call\n")
	...
	b.WriteString("  q1: browser.fetch → get page text\n")
