...
func buildPlannerPrompt(instruction string, maxQuests int) (system, user string) {
	...
	b.WriteString("This node has 3 Organic Tools — all available, wake on demand:\n\n")
	b.WriteString("🌐 Vātāyana (browser) — LIVE web access via Firefox\n")
	b.WriteString("   browser.fetch  → GET a URL, returns page text + all links\n")
	b.WriteString("   browser.navigate → open URL in visible Firefox\n")
	b.WriteString("   browser.window  → current window title\n")
	b.WriteString("🛠️ Yantraśālā (sandbox) — persistent Linux shell + filesystem\n")
	b.WriteString("   sandbox.exec      → run any command (curl, python3, jq, sqlite3...)\n")
	b.WriteString("   sandbox.shell     → persistent session (cwd/env survive)\n")
	b.WriteString("   sandbox.file_write/read → read/write files\n")
	b.WriteString("   sandbox.tree      → filesystem tree\n")
	b.WriteString("📚 Vault (research) — stored markdown notes (FTS5 search)\n")
	b.WriteString("   research.search   → full-text search saved notes\n")
	b.WriteString("   research.notes    → list recent notes\n")
	b.WriteString("\nAll tools listed below:\n")
	for _, t := range karaka.ListTools() {
		fmt.Fprintf(&b, "- %s — %s\n", t.ID, t.Description)
	}
	b.WriteString("\n---\n")
	fmt.Fprintf(&b, "Plan between 1-%d quests. Output STRICT JSON:\n", maxQuests)
	b.WriteString("{\"quests\": [{\"title\": \"...\", \"tool\": \"browser.fetch\", \"params\": {\"url\": \"https://...\"}, \"depends_on\": []}]}\n")
	b.WriteString("No markdown, no commentary — ONLY JSON.\n")
